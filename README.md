...
```bash
psql -U postgres -c "DROP DATABASE IF EXISTS tiny_forum;"
...
```bash
# 使用 etcd 配置中心
go run cmd/server/main.go --use-etcd=true --env=dev

# 指定 etcd 配置文件
go run cmd/server/main.go --use-etcd=true --etcd-config=config/etcd.yml --env=prod
```
