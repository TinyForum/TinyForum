...
type Report struct {
	...
	TargetType ReportTargetType `gorm:"column:target_type;size:50;not null;index;comment:被举报对象类型(post/comment/user)" json:"target_type"` // 被举报对象类型
...
const (
	...
	ReportTargetTypeUser    ReportTargetType = "user"    // 用户
)
...
// enum [spam, offensive, illegal, misinformation, privacy, other]
