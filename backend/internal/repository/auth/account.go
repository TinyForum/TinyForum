...
func (r *authRepository) HardDelete(ctx context.Context, id uint) error {
	return r.db.WithContext(ctx).Unscoped().Delete(&do.User{}, id).Error
}

// HardDeleteWithTx 事务中硬删除
func (r *authRepository) HardDeleteWithTx(ctx context.Context, tx *gorm.DB, id uint) error {
	return tx.WithContext(ctx).Unscoped().Delete(&do.User{}, id).Error
}

// GetDeletedUser 获取已软删除的用户
func (r *authRepository) GetDeletedUser(ctx context.Context, id uint) (*do.User, error) {
	var user do.User
...
func (r *authRepository) GetUserWithDeleted(ctx context.Context, id uint) (*do.User, error) {
	var user do.User
