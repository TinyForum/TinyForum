...
func (r *authRepository) Restore(ctx context.Context, id uint) error {
	return r.db.Unscoped().Model(&model.User{}).
		Where("id = ?", id).
...
func (r *authRepository) HardDeleteWithTx(ctx context.Context, tx *gorm.DB, id uint) error {
	return tx.WithContext(ctx).Unscoped().Delete(&model.User{}, id).Error
}

// GetDeletedUser 获取已软删除的用户
func (r *authRepository) GetDeletedUser(ctx context.Context, id uint) (*model.User, error) {
	var user model.User
...
func (r *authRepository) GetUserWithDeleted(ctx context.Context, id uint) (*model.User, error) {
	var user model.User
