...
type AuthRepository interface {
	FindByEmail(ctx context.Context, email string) (*po.User, error)              // 通过邮箱查找用户
	Update(ctx context.Context, user *po.User) error                              // 更新用户信息
	Save(ctx context.Context, user *po.User) error                                // 保存用户信息
	...
	GetDeletedUser(ctx context.Context, id uint) (*po.User, error)                // 获取软删除用户
	GetUserWithDeleted(ctx context.Context, id uint) (*po.User, error)            // 获取包含软删除信息的用户
	GetUserByResetToken(ctx context.Context, token string) (*po.User, error)      // 通过重置密码令牌查找用户
	...
	FindByResetToken(ctx context.Context, token string) (*po.RefreshToken, error) // 通过重置密码令牌查找用户
