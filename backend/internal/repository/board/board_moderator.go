...
// AddModerator 添加版主
func (r *boardRepository) AddModerator(mod *do.Moderator) error {
	return r.db.Create(mod).Error
}

// UpdateModerator 更新版主权限
func (r *boardRepository) UpdateModerator(mod *do.Moderator) error {
...
func (r *boardRepository) FindModeratorByUserAndBoard(userID, boardID uint) (*do.Moderator, error) {
	var mod do.Moderator
...
func (r *boardRepository) GetModerators(boardID uint) ([]do.Moderator, error) {
	var mods []do.Moderator
...
func (r *boardRepository) CreateModeratorLog(log *do.ModeratorLog) error {
	return r.db.Create(log).Error
}

// GetModeratorLogs 获取版主操作日志
func (r *boardRepository) GetModeratorLogs(boardID uint, limit, offset int) ([]do.ModeratorLog, int64, error) {
	var logs []do.ModeratorLog
	var total int64

	query := r.db.Model(&do.ModeratorLog{}).Where("board_id = ?", boardID)
	query.Count(&total)
