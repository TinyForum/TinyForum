...
type PluginRepository interface {
	...
	ListByAuthorID(ctx context.Context, authorID uint) ([]*do.PluginMeta, error)
	...
	IsExist(ctx context.Context, name string) (bool,error)
	 SoftDeleteByID(ctx context.Context, id uint) error
	  GetByID(ctx context.Context, id uint) (*do.PluginMeta, error) 
	  FindByNameUnscoped(ctx context.Context, name string) (*do.PluginMeta, error)
	    DeletePermanentlyByID(ctx context.Context, id uint) error   
}
...
// SoftDeleteByID 软删除插件（设置 deleted_at）
func (r *pluginRepo) SoftDeleteByID(ctx context.Context, id uint) error {
    return r.db.WithContext(ctx).
        Where("id = ?", id).
        Delete(&do.PluginMeta{}).Error // GORM 自动设置 deleted_at
}