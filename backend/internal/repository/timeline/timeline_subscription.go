...
func (r *timelineRepository) Subscribe(sub *po.TimelineSubscription) error {
	var existing po.TimelineSubscription
...
func (r *timelineRepository) Unsubscribe(subscriberID, targetUserID uint) error {
	return r.db.Model(&po.TimelineSubscription{}).
		Where("subscriber_id = ? AND target_user_id = ?", subscriberID, targetUserID).
		Update("is_active", false).Error
}

// GetSubscriptions 获取用户的所有有效关注
func (r *timelineRepository) GetSubscriptions(subscriberID uint) ([]po.TimelineSubscription, error) {
	var subs []po.TimelineSubscription
...
func (r *timelineRepository) IsSubscribed(subscriberID, targetUserID uint) (bool, error) {
	var count int64
	err := r.db.Model(&po.TimelineSubscription{}).
