...
type UserRepository interface {
	...
	//  crud
	Create(user *do.User) error                                              // 创建用户
	FindByID(id uint) (*do.User, error)                                      // 根据ID查找用户
	FindByEmail(ctx context.Context, email string) (*do.User, error)         // 根据邮箱查找用户
	FindByUsername(username string) (*do.User, error)                        // 根据用户名查找用户
	Update(ctx context.Context, user *do.User) error                         // 更新用户
	UpdateFields(id uint, fields map[string]any) error                       // 更新用户字段
	List(page, pageSize int, keyword string) ([]do.User, int64, error)       // 分页查询用户
	FindByIDs(ids []uint) ([]do.User, error)                                 // 根据ID列表查找用户
	GetUserBasicInfo(id uint) (*do.User, error)                              // 获取用户基本信息
	GetUserBasicInfoById(userID uint) (*do.User, error)                      // 根据ID获取用户基本信息
	GetUserRoleById(userID uint) (string, error)                             // 根据ID获取用户角色
	FindByEmailUnscoped(ctx context.Context, email string) (*do.User, error) // 根据邮箱查找用户（不使用软删除）
	// follow
	...
	GetFollowers(userID uint, page, pageSize int) ([]do.User, int64, error) // 获取关注者列表
	GetFollowing(userID uint, page, pageSize int) ([]do.User, int64, error) // 获取关注列表
	// rank
