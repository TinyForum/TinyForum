...
func (s *authService) ConfirmDeletion(ctx context.Context, userID uint) error {
	return s.txManager.ExecuteInTransaction(ctx, func(tx *gorm.DB) error {
		...
		// if err := tx.Where("user_id = ? OR sender_id = ?", userID, userID).Delete(&model.Notification{}).Error; err != nil {
		// 	return fmt.Errorf("删除通知失败: %w", err)
		// }
		// 2. 点赞
		...
		// 3. 投票
		if err := tx.Where("user_id = ?", userID).Delete(&model.Vote{}).Error; err != nil {
			return fmt.Errorf("删除投票失败: %w", err)
		}

		// 4. 回答投票
		if err := tx.Where("user_id = ?", userID).Delete(&model.AnswerVote{}).Error; err != nil {
			return fmt.Errorf("删除回答投票失败: %w", err)
		}

		// 5. 关注关系（作为关注者）
		if err := tx.Where("follower_id = ?", userID).Delete(&model.Follow{}).Error; err != nil {
			return fmt.Errorf("删除关注关系失败: %w", err)
		}

		// 6. 签到记录
		if err := tx.Where("user_id = ?", userID).Delete(&model.SignIn{}).Error; err != nil {
			return fmt.Errorf("删除签到记录失败: %w", err)
		}

		// 7. 版主申请
		if err := tx.Where("user_id = ? OR reviewer_id = ?", userID, userID).Delete(&model.ModeratorApplication{}).Error; err != nil {
			return fmt.Errorf("删除版主申请失败: %w", err)
		}

		// 8. 版主记录
		if err := tx.Where("moderator_id = ?", userID).Delete(&model.ModeratorLog{}).Error; err != nil {
			return fmt.Errorf("删除版主日志失败: %w", err)
		}

		// 9. 版主权限
		if err := tx.Where("user_id = ?", userID).Delete(&model.Moderator{}).Error; err != nil {
			return fmt.Errorf("删除版主权限失败: %w", err)
		}

		// 10. 举报（作为举报人或处理人）
		if err := tx.Where("reporter_id = ? OR handler_id = ?", userID, userID).Delete(&model.Report{}).Error; err != nil {
			return fmt.Errorf("删除举报记录失败: %w", err)
		}

		// 11. 时间线事件（作为用户或触发者）
		if err := tx.Where("user_id = ? OR actor_id = ?", userID, userID).Delete(&model.TimelineEvent{}).Error; err != nil {
			return fmt.Errorf("删除时间线事件失败: %w", err)
		}

		// 12. 时间线订阅
		if err := tx.Where("subscriber_id = ? OR target_user_id = ?", userID, userID).Delete(&model.TimelineSubscription{}).Error; err != nil {
			return fmt.Errorf("删除时间线订阅失败: %w", err)
		}

		// 13. 用户时间线
		if err := tx.Where("user_id = ?", userID).Delete(&model.UserTimeline{}).Error; err != nil {
			return fmt.Errorf("删除用户时间线失败: %w", err)
		}

		// 14. 主题关注
		if err := tx.Where("user_id = ?", userID).Delete(&model.TopicFollow{}).Error; err != nil {
			return fmt.Errorf("删除主题关注失败: %w", err)
		}

		// 15. 主题帖子关联
		if err := tx.Where("added_by = ?", userID).Delete(&model.TopicPost{}).Error; err != nil {
			return fmt.Errorf("删除主题帖子关联失败: %w", err)
		}

		// 16. 主题（创建者）
		if err := tx.Where("creator_id = ?", userID).Delete(&model.Topic{}).Error; err != nil {
			return fmt.Errorf("删除主题失败: %w", err)
		}

		// 17. 版块封禁
		if err := tx.Where("user_id = ? OR banned_by = ?", userID, userID).Delete(&model.BoardBan{}).Error; err != nil {
			return fmt.Errorf("删除版块封禁失败: %w", err)
		}

		// 18. 公告（创建者或更新者）
		if err := tx.Where("created_by = ? OR updated_by = ?", userID, userID).Delete(&model.Announcement{}).Error; err != nil {
			return fmt.Errorf("删除公告失败: %w", err)
		}

		// 19. 邀请关系
		if err := tx.Model(&model.User{}).Where("invited_by_id = ?", userID).Update("invited_by_id", nil).Error; err != nil {
		...
		// 20. 评论（置空 author_id）
		if err := tx.Model(&model.Comment{}).Where("author_id = ?", userID).Update("author_id", nil).Error; err != nil {
			return fmt.Errorf("更新评论失败: %w", err)
		}

		// 21. 帖子（置空 author_id）
		if err := tx.Model(&model.Post{}).Where("author_id = ?", userID).Update("author_id", nil).Error; err != nil {
