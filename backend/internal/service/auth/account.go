...
func (s *authService) ConfirmDeletion(ctx context.Context, userID uint) error {
	return s.txManager.ExecuteInTransaction(ctx, func(tx *gorm.DB) error {
		...
		// if err := tx.Where("user_id = ? OR sender_id = ?", userID, userID).Delete(&do.Notification{}).Error; err != nil {
		// 	return fmt.Errorf("删除通知失败: %w", err)
		// }
		// 2. 点赞
		if err := tx.Where("user_id = ?", userID).Delete(&do.Like{}).Error; err != nil {
		...
		// 5. 关注关系（作为关注者）
		if err := tx.Where("follower_id = ?", userID).Delete(&do.Follow{}).Error; err != nil {
			return fmt.Errorf("删除关注关系失败: %w", err)
		}

		// 6. 签到记录
		if err := tx.Where("user_id = ?", userID).Delete(&do.SignIn{}).Error; err != nil {
			return fmt.Errorf("删除签到记录失败: %w", err)
		}

		// 7. 版主申请
		if err := tx.Where("user_id = ? OR reviewer_id = ?", userID, userID).Delete(&do.ModeratorApplication{}).Error; err != nil {
			return fmt.Errorf("删除版主申请失败: %w", err)
		}

		// 8. 版主记录
		if err := tx.Where("moderator_id = ?", userID).Delete(&do.ModeratorLog{}).Error; err != nil {
			return fmt.Errorf("删除版主日志失败: %w", err)
		}

		// 9. 版主权限
		if err := tx.Where("user_id = ?", userID).Delete(&do.Moderator{}).Error; err != nil {
			return fmt.Errorf("删除版主权限失败: %w", err)
		}

		// 10. 举报（作为举报人或处理人）
		if err := tx.Where("reporter_id = ? OR handler_id = ?", userID, userID).Delete(&do.Report{}).Error; err != nil {
			return fmt.Errorf("删除举报记录失败: %w", err)
		}

		// 11. 时间线事件（作为用户或触发者）
		if err := tx.Where("user_id = ? OR actor_id = ?", userID, userID).Delete(&do.TimelineEvent{}).Error; err != nil {
		...
		// 13. 用户时间线
		if err := tx.Where("user_id = ?", userID).Delete(&do.UserTimeline{}).Error; err != nil {
			return fmt.Errorf("删除用户时间线失败: %w", err)
		}

		// 14. 主题关注
		if err := tx.Where("user_id = ?", userID).Delete(&do.TopicFollow{}).Error; err != nil {
		...
		// 16. 主题（创建者）
		if err := tx.Where("creator_id = ?", userID).Delete(&do.Topic{}).Error; err != nil {
			return fmt.Errorf("删除主题失败: %w", err)
		}

		// 17. 版块封禁
		if err := tx.Where("user_id = ? OR banned_by = ?", userID, userID).Delete(&do.BoardBan{}).Error; err != nil {
			return fmt.Errorf("删除版块封禁失败: %w", err)
		}

		// 18. 公告（创建者或更新者）
		if err := tx.Where("created_by = ? OR updated_by = ?", userID, userID).Delete(&do.Announcement{}).Error; err != nil {
			return fmt.Errorf("删除公告失败: %w", err)
		}

		// 19. 邀请关系
		if err := tx.Model(&do.User{}).Where("invited_by_id = ?", userID).Update("invited_by_id", nil).Error; err != nil {
		...
		// 20. 评论（置空 author_id）
		if err := tx.Model(&do.Comment{}).Where("author_id = ?", userID).Update("author_id", nil).Error; err != nil {
		...
		// 23. 删除用户
		if err := tx.Unscoped().Delete(&do.User{}, userID).Error; err != nil {
