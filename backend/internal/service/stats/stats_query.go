...
func (s *statsService) GetStatsByDate(ctx context.Context, date time.Time, statsType string) (*dto.StatsTodayInfo, error) {
	startOfDay := time.Date(date.Year(), date.Month(), date.Day(), 0, 0, 0, 0, date.Location())
	endOfDay := time.Date(date.Year(), date.Month(), date.Day(), 23, 59, 59, 999999999, date.Location())

	// 统一返回完整结构
	info := &dto.StatsTodayInfo{}
...
func (s *statsService) GetTotalStats(ctx context.Context, startDate, endDate time.Time, statsType string) (*dto.StatsInfoResp, error) {
	fmt.Printf("GetTotalStats: start_date=%s, end_date=%s, stats_type=%s", startDate, endDate, statsType)
	resp := &dto.StatsInfoResp{StatTime: time.Now()}
	...
	case "users":
		...
		resp.TodayInfo = &dto.StatsTodayInfo{NewUser: count}
	case "posts":
		count, err := s.postRepo.CountByDateRange(ctx, startDate, endDate)
		if err != nil {
			return nil, err
		}
		resp.TodayInfo = &dto.StatsTodayInfo{NewArticle: count}
	case "comments":
		count, err := s.commentRepo.CountByDateRange(ctx, startDate, endDate)
		if err != nil {
			return nil, err
		}
		resp.TodayInfo = &dto.StatsTodayInfo{NewComment: count}
	default: // "all"
...
func (s *statsService) GetTrendStats(ctx context.Context, startDate, endDate time.Time, statsType, intervals string) ([]*dto.TrendData, error) {
	dates := generateDateRange(startDate, endDate, intervals)
	trendData := make([]*dto.TrendData, 0, len(dates))
	for _, date := range dates {
		...
		trendData = append(trendData, &dto.TrendData{
