...
type TopicService interface {
	Create(creatorID uint, input CreateTopicInput) (*model.Topic, error)
	Update(id uint, input CreateTopicInput) (*model.Topic, error)
	Delete(id uint, userID uint, isAdmin bool) error
	GetByID(id uint) (*model.Topic, error)
	List(page, pageSize int) ([]model.Topic, int64, error)
	GetByCreator(creatorID uint, page, pageSize int) ([]model.Topic, int64, error)
	// follow
	Follow(userID, topicID uint) error
	Unfollow(userID, topicID uint) error
	IsFollowing(userID, topicID uint) (bool, error)
	GetFollowers(topicID uint, page, pageSize int) ([]model.TopicFollow, int64, error)
	// post
	AddPostToTopic(input AddPostToTopicInput, userID uint) error
	RemovePostFromTopic(topicID, postID uint, userID uint) error
	GetTopicPosts(topicID uint, page, pageSize int) ([]model.TopicPost, int64, error)
}
