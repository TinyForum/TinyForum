...
const (
	...
	LevelShadow  Level = 3 // 【防对抗】遮蔽词：发布者本人可见，其他用户不可见。例："加微信"、"点击链接"、"私聊我"（高频广告词或试探性违规，让用户以为发布成功但实际不扩散，减少对抗。）
	LevelBlock   Level = 4 // 【危险】直接拦截：拒绝发布并提示违规。例："法轮功"、"六四"、"贩卖毒品"、"儿童色情"（法律法规明确禁止的内容，必须彻底拦截。）
)
