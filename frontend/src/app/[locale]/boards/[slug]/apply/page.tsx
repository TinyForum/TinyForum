...
export default function ApplyModeratorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed) { setError('请填写申请理由'); return; }
    if (trimmed.length < 20) { setError('申请理由至少需要 20 个字符'); return; }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
            ...
            <div className="rounded-xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/40 p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-2 flex-1">
                  <p className="font-medium text-gray-900 dark:text-white">你已提交过申请</p>
                  <div className="text-sm text-gray-500 dark:text-gray-400 space-y-1">
                    <p>提交时间：{new Date(existing.created_at).toLocaleString('zh-CN')}</p>
                  ...
                  {existing.status === 'approved' && (
                    <p className="text-sm text-green-600 dark:text-green-400">🎉 恭喜！你已成为该板块的版主。</p>
                  )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={label} className="text-center bg-gray-50 dark:bg-gray-700/50 rounded-xl py-3">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">版主职责</p>
                <ul className="text-sm text-gray-500 dark:text-gray-400 space-y-1">
                  {['维护板块秩序，删除违规内容', '管理优质内容，设置精华和置顶', '及时回复用户问题，提供帮助', '处理举报，维护社区和谐'].map(r => (
              ...
              <div>
                ...
                <div className="flex justify-between mt-1 text-xs">
                  ...
                  <span className={reason.trim().length < 20 ? 'text-gray-300' : 'text-green-500'}>
              ...
              <div className="flex items-center justify-between pt-1">
                {reapply ? (
                  <button
                    type="button"
                    onClick={() => { setReapply(false); setError(''); }}
                    className="text-sm text-gray-400 hover:text-gray-600 transition-colors"
                  >
                    取消重新申请
                  </button>
                ) : (
                  <Link href={`/boards/${slug}`} className="text-sm text-gray-400 hover:text-blue-500 transition-colors">
                    取消
                  </Link>
                )}
                <button
      ...
      <div className="mt-5 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4">
        <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">温馨提示</p>
