...
export default function UserProfileClient({ userId }: { userId: number }) {
  ...
  const followMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(profile?.is_following ? t("unfollowed") : t("followed"));
    },
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <div className="flex items-center justify-center sm:justify-start gap-6 mt-4">
                <div className="text-center">
                  ...
                  <div className="text-xs text-base-content/40 flex items-center gap-1"><Star className="w-3 h-3" /> {t("score")}</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold">{profile.follower_count ?? 0}</div>
                  <div className="text-xs text-base-content/40 flex items-center gap-1"><Users className="w-3 h-3" /> {t("followers")}</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold">{profile.following_count ?? 0}</div>
                  <div className="text-xs text-base-content/40 flex items-center gap-1"><Users className="w-3 h-3" /> {t("following")}</div>
                </div>
              </div>
            </div>
