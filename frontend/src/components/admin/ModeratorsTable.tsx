...
import { Shield, UserPlus, UserMinus, Edit, Search, ShieldCheck, ShieldAlert, ShieldX, ChevronLeft, ChevronRight } from "lucide-react";
import { moderatorApi, Moderator, AddModeratorRequest } from "@/lib/api/modules/moderator";
...
export function ModeratorsTable() {
  const { boards, loading: boardsLoading } = useBoard({ autoLoad: true, pageSize: 500 });
  const [moderators, setModerators] = useState<ExtendedModerator[]>([]);
  const [loading, setLoading] = useState(true);
  const [showAddModal, setShowAddModal] = useState(false);
  const [showEditModal, setShowEditModal] = useState(false);
  const [selectedModerator, setSelectedModerator] = useState<ExtendedModerator | null>(null);
  ...
  const [selectedBoardId, setSelectedBoardId] = useState<number>(boards[0]?.id || 0);
  ...
  const filteredModerators = moderators.filter(moderator => {
    // 板块筛选
    if (selectedBoardFilter !== "all" && moderator.board_id !== selectedBoardFilter) {
      return false;
    }
    // 关键词筛选
    if (searchKeyword && !moderator.user?.username?.toLowerCase().includes(searchKeyword.toLowerCase()) &&
  ...
  const stats = {
    ...
    permissionsStats: {
      canDeletePost: moderators.filter(m => m.permissions?.can_delete_post).length,
  ...
  const handleRemoveModerator = async (moderator: ExtendedModerator) => {
    if (!confirm(`确定要移除 ${moderator.user?.username || moderator.user_id} 的版主权限吗？`)) return;
  ...
  const PermissionBadge = ({ label, hasPermission }: { label: string; hasPermission: boolean }) => (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
      ...
      {hasPermission ? <ShieldCheck className="w-3 h-3" /> : <ShieldX className="w-3 h-3" />}
      {label}
    </span>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="stat bg-base-100 rounded-lg border border-base-300">
          ...
          <div className="stat-value text-secondary">{stats.permissionsStats.canDeletePost}</div>
        </div>
        <div className="stat bg-base-100 rounded-lg border border-base-300">
          ...
          <div className="stat-value text-accent">{stats.permissionsStats.canBanUser}</div>
        </div>
        <div className="stat bg-base-100 rounded-lg border border-base-300">
          ...
          <div className="flex flex-wrap justify-between items-center gap-4 mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <select 
                ...
                onChange={(e) => {
                  setSelectedBoardFilter(e.target.value === "all" ? "all" : Number(e.target.value));
                  setCurrentPage(1);
                }}
              >
                      ...
                      <td>
                        <div className="flex items-center gap-2">
                          {moderator.user?.avatar && (
                            <img src={moderator.user.avatar} alt="" className="w-8 h-8 rounded-full" />
                          )}
                          <div>
                            <div className="font-medium">{moderator.user?.username || `用户 #${moderator.user_id}`}</div>
                            <div className="text-xs text-base-content/50">ID: {moderator.user_id}</div>
                          </div>
                        </div>
                      </td>
                      <td>
                        <span className="badge badge-ghost">
                          {moderator.board_name || `板块 #${moderator.board_id}`}
                        </span>
                      </td>
                      <td>
                        <div className="flex flex-wrap gap-1">
                          <PermissionBadge label="删帖" hasPermission={moderator.permissions?.can_delete_post} />
                          <PermissionBadge label="置顶" hasPermission={moderator.permissions?.can_pin_post} />
                          <PermissionBadge label="编辑" hasPermission={moderator.permissions?.can_edit_any_post} />
                          <PermissionBadge label="管理版主" hasPermission={moderator.permissions?.can_manage_moderator} />
                          <PermissionBadge label="禁言" hasPermission={moderator.permissions?.can_ban_user} />
                        </div>
                      </td>
          ...
          <div className="modal-box">
            ...
            <div className="modal-action">
              ...
              <button 
                ...
                disabled={submitting}
              >
                {submitting ? <span className="loading loading-spinner loading-sm" /> : "确认"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="modal modal-open">
          <div className="modal-box">
            <h3 className="font-bold text-lg">
              编辑权限 - {selectedModerator.user?.username || `用户 #${selectedModerator.user_id}`}
            </h3>
            <p className="text-sm text-base-content/60 mt-1">
              板块：{selectedModerator.board_name || `板块 #${selectedModerator.board_id}`}
            </p>
            ...
            <div className="modal-action">
              ...
              <button 
                ...
                disabled={submitting}
              >
                {submitting ? <span className="loading loading-spinner loading-sm" /> : "保存"}
              </button>
            </div>
          </div>
        </div>
