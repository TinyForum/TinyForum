...
export function TagSelector({ tags, selectedTags, onToggleTag, loading, maxTags = 5 }: TagSelectorProps) {
    ...
    return (
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="h-8 w-16 bg-base-200 rounded-full animate-pulse" />
          ))}
        </div>
      </div>
    );
