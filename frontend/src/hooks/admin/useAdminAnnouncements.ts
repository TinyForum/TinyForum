...
interface UseAdminAnnouncementsReturn {
  ...
  createAnnouncement: (data: CreateAnnouncementPayload) => Promise<Announcement | null>;
  updateAnnouncement: (id: number, data: UpdateAnnouncementPayload) => Promise<Announcement | null>;
...
export function useAdminAnnouncements(): UseAdminAnnouncementsReturn {
  ...
  const fetchAnnouncements = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.code === 200 || response.data.code === 0) {
        ...
        // 筛选置顶公告
        setPinnedAnnouncements(list.filter((ann: Announcement) => ann.is_pinned === true));
      } else {
  ...
  // 根据 ID 获取公告详情
  const getAnnouncementById = useCallback(async (id: number): Promise<Announcement | null> => {
    try {
  ...
  // 创建公告
  const createAnnouncement = useCallback(async (data: CreateAnnouncementPayload): Promise<Announcement | null> => {
  ...
  // 更新公告
  const updateAnnouncement = useCallback(async (id: number, data: UpdateAnnouncementPayload): Promise<Announcement | null> => {
    setIsSubmitting(true);
    try {
  ...
  const publishAnnouncement = useCallback(async (id: number): Promise<boolean> => {
    setIsSubmitting(true);
    try {
  ...
  const archiveAnnouncement = useCallback(async (id: number): Promise<boolean> => {
    setIsSubmitting(true);
    try {
      const response = await announcementApi.update(id, {
  ...
  const pinAnnouncement = useCallback(async (id: number, isPinned: boolean): Promise<boolean> => {
    setIsSubmitting(true);
    try {
      const response = await announcementApi.update(id, { is_pinned: isPinned });
      if (response.data.code === 200 || response.data.code === 0) {
