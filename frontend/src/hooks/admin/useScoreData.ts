...
export function useScoreData(userId?: string | undefined) {
  ...
  const { data: scoreRecords, isLoading: isLoadingRecords } = useQuery({
    ...
    queryFn: () => scoreApi.getAllUserScore({ id: userId ? Number(userId) : undefined }),
    enabled: true,
  });
  ...
  const setScoreMutation = useMutation({
    mutationFn: ({ userId, score, reason }: { userId: number; score: number; reason: string }) =>
  ...
  const addScoreMutation = useMutation({
    mutationFn: ({ userId, increment, reason }: { userId: number; increment: number; reason: string }) =>
  ...
  const subtractScoreMutation = useMutation({
    mutationFn: ({ userId, decrement, reason }: { userId: number; decrement: number; reason: string }) =>
