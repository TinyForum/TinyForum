...
export function useStatsData(enabled: boolean = true) {
  ...
  const stats = (() => {
    ...
    return {
      ...
      postGrowth: calculateGrowth(todayInfo?.new_article || 0, previousNewArticle),
  ...
  const charts = (() => {
    return {
      userGrowthTrend: userTrend?.data.data.trend?.map(item => ({
        date: item.date,
        value: item.count
      })) || [],
      postTrend: postTrend?.data.data.trend?.map(item => ({
        date: item.date,
        value: item.count
      })) || [],
    };
  ...
  // 预设时间范围
  const setPresetRange = useCallback((range: 'today' | 'week' | 'month' | 'year') => {
    const end = new Date();
    const start = new Date();
    
    switch (range) {
      case 'today':
        updateDateRange(end.toISOString().split('T')[0], end.toISOString().split('T')[0]);
        break;
      case 'week':
