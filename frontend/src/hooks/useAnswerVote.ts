...
export function useAnswerVote(answerId: number, currentUserId?: number) {
  ...
  const loadVoteStatus = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await answerApi.getVoteStatus(answerId);
      ...
      if (response.data.code === 0) {
        ...
        const newUserVote = userVoteValue === 1 ? 'up' : userVoteValue === -1 ? 'down' : '';
        setUserVote(newUserVote);
        setVoteCount(netVotes);
        
        console.log(`Answer ${answerId} - user_vote: ${userVoteValue}, up: ${upCount}, down: ${downCount}, net: ${netVotes}`);
      }
    } catch (error) {
...
const handleVote = useCallback(async (voteType: 'up' | 'down') => {
  if (!currentUserId) { toast.error('请先登录'); return false; }
